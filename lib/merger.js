...
function merge(savedJsonData, newJsonData) {
  ...
  mergedJsonData.collection.info = newJsonData.collection.info;
  if (newJsonData.collection.event) {
    mergedJsonData.collection.event = newJsonData.collection.event;
  }
  if (newJsonData.collection.variable) {
    var savedVars = mergedJsonData.collection.variable || [];
    var savedKeys = new Set(savedVars.map(v => v.key));
    newJsonData.collection.variable.forEach(v => {
      if (!savedKeys.has(v.key)) {
        savedVars.push(v);
      }
    });
    mergedJsonData.collection.variable = savedVars;
  }
